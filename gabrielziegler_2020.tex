...
% brief introduction connecting main topics
...
\begin{figure}[H]
    ...
    \caption{Schematic of an Artificial Neuron\cite{oliveira_system_2017}.}\label{visina8}
\end{figure}
...
The idea came from questioning the necessity of extracting large portions of samples when much of these samples are discarded, exposing the inefficiency of trying to gather all signal.
...
\ac{CS} parts from the principle that if given $x$, a digital image or signal has a sparse representation in a orthonormal basis (e.g.wavelet, Fourier), then the $N$ most important coeffiecients in that expasion allow reconstruction with $l_2$ error $O(N^{1/2-1/p})$\cite{donoho_compressed_2006}.
