...
% \subsection{Pre Filtered Phantom Mask}
% 
% The pre-filtered images represent horizontal, vertical and diagonal borders masks produced from the element-wise multiplication described above and these are the masks that we will later use as our support position candidates.
% 
% \begin{figure}[H]
%   \centering
%   \begin{minipage}[t]{0.3\textwidth}
%       \includegraphics[width=\textwidth]{filtered_phantom_hor}
%   \end{minipage}
%   \begin{minipage}[t]{0.3\textwidth}
%       \includegraphics[width=\textwidth]{filtered_phantom_ver}
%   \end{minipage}
%   \begin{minipage}[t]{0.3\textwidth}
%       \includegraphics[width=\textwidth]{filtered_phantom_diag}
%   \end{minipage}
%   \caption{Phantom borders filtered with 3\% of deterministic prior information extraction.}
% \end{figure}
% 
% It becomes clearer when the masks above are observed that by using deterministic prior information, where all positions have the same likelyhood to belong to a support structure will induce some error, as we can note some artifacts generated in the above images for an example as simple as a phantom.
...
Although we are still using a great amount of prior information proportion, the regions that should have a high confidence of belonging to support structures in our subject are much clearer visually now when $\Phi$ values are weighted by our weights matrix $x$~\ref{eqn:reweighted}.
This clarity observed in the visualizations is also reflected in the $l1$-minimization iteractions as can be observed in the results~\ref{tab:dynamic-cardiac}.
...
\begin{figure}
    ...
    \centering
    \subfloat[]{\label{main:c}\includegraphics[width=0.49\textwidth]{cardiac_frame1}}
    \caption{my fig}
    \label{fig:main}
\end{figure}
