...
\ac{CS} theory provides accurate reconstruction of unknown sparse signals from underdetermined linear measurements $l$.
...
In \ac{CS} theory, this is accomplished by using an optimization algorithm with restrictions that exploits the sparsity in the domain defined by $T$.
...
The $\ell_p$-minimization is an optimization algorithm that is a rather common approach used in reconstruction procedure, specially the $\ell_1$ regularization, as it often produces better results.
...
\noindent
The solution to the \ref{eq:direct_method} is extremely resource consuming and is not viable in most practical situations, \ac{MRI} reconstructions included.
