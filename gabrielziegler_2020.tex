...
By taking this into account, we segmented the image into four levels of movement -- \textit{high}, \textit{medium}, \textit{low} and \textit{close-to-zero} movement.
...
% TODO
Our experiments were conducted with manually segmented images in the according levels. The regions labelled as \textit{high} movement receive a covariance matrix with higher variance; thus assigning this region as a low-confidence support position, whilst the regions labelled as \textit{low} or \textit{close-to-zero} receive a Gaussian distribution with a low variance, representing it as a highly likely support position prior information.
